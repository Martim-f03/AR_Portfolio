...
# ── Main training loop (Com Alpha Annealing incorporado) ──────────────────────
...
def train(
    ...
    Includes Linear Alpha Annealing to decay the learning rate dynamically.
    """
    env = TicTacToeEnv()
    ...
    # Inicializar hiperparâmetros de decaimento para o Alpha
    agent.alpha = initial_alpha
    decay_per_episode = (initial_alpha - min_alpha) / num_episodes
    ...
    for ep in range(1, num_episodes + 1):
        ...
        # Aplicar o Decaimento Linear do Alpha após cada episódio
        agent.alpha = max(min_alpha, agent.alpha - decay_per_episode)
