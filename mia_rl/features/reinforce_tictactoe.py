...
def run_reinforce_episode(
    ...
    state = env.reset()
    done = False  # Controla o fim do jogo por aqui

    traj_x: list[tuple[np.ndarray, int, list[int], float]] = []
    traj_o: list[tuple[np.ndarray, int, list[int], float]] = []

    while not done:  # Substituído env.is_terminal(state) por not done
...
def train(
    ...
    # Configuração inicial do decaimento linear da taxa de aprendizagem
    ...
    for ep in range(1, num_episodes + 1):
        ...
        # Aplicar o decaimento após cada episódio
