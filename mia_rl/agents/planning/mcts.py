...
class MCTSAgent:
    ...
    def _rollout(self, state: TicTacToeState, player: int) -> float:
        ...
        while not _is_terminal(current_state):
            actions = _available(current_state)
            action = random.choice(actions)
            current_state = _apply(current_state, action, current_player)
            current_player = -current_player

        # TODO (3/3): Return the outcome from the original ``player``'s perspective.
        winner = _winner(current_state)
